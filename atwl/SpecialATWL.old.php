...
class SpecialATWL extends SpecialPage {
	...
	public function execute($p) {
		...
		if ($passed == '') {
			...
			$selects = implode(array_map(create_function('$f','return "<option value=\'$f\' ".($f=="'.$format.'"?"selected":"").">$f</option>";'), $formats));
			$formatSelector = "<form><label for='format' style='text-align:left; vertical-align:middle;'>Format: </label><select id='format' name='format' onchange='changeFormat(this);'>$selects</select></form>";
			
			$formatSelector .= SMWAskPage::showFormatOptions($format, $params);
			
			/*$formatSelector = SMWAskPage::getInputForm($result['printouts'],
									'offset=' . $params['offset'] . '&limit=' . $params['limit'] . $urltail);
*/
