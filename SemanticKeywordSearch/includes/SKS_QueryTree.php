...
/**
 *  A query interpretation consists of an array of SKSKeyword objects
 */
class SKSKeyword {
...
class SKSQueryTree {
	...
	public function outputInterpretations() {
		...
		foreach ($this->paths as $path) {
			...
			$query = SKSSpecialPage::getAskQuery($path);
			$mainlabel = $query['mainlabel'];
			
			$query = $query['result'];
			$result = SKSSpecialPage::getAskQueryResult($query);
			...
			$result = preg_replace_callback("/\<tr\>(.+?)\<\/th\>/si",
				create_function('$m',
					'return "<tr>$m[1]<a href=\"'.$link.'\"><img style=\"float:left;\" src=\"../extensions/SemanticKeywordSearch/magnifier.png\"></a></th>";'
				),
				$result
			);
		...
		$intro = "<ul><li>Your search returned $count interpretations.</li><li>Choose the interpretation that fits your needs best by clicking on the magnifying glass.</li><li>Note: You can add and remove properties in the next step.</li></ul>";
		return $intro . $m;
	}
	...
	protected function paths(&$node, $expectTypes) {
		...
		foreach ($node->current->types as &$type) {
			foreach ($node->children as &$child) {
				if ($a = array_intersect($child->current->types, SKSQueryTree::$atwExpectTypes[$type])) {
					foreach ($this->paths($child, $a) as $intr) {
						if (in_array($intr[0]->type, SKSQueryTree::$atwExpectTypes[$type])) {
							$ret[] = array_merge(array(new SKSKeyword($node->current->kwString, $type)), $intr);
						}
					}				
				}
			}			
...
class SKSQueryNode {
	...
	function __construct($current, $remaining) {			
		...
		$nextExpect = array();				
		foreach (SKSQueryTree::$atwExpectTypes as $type => $validfollowers) {			
